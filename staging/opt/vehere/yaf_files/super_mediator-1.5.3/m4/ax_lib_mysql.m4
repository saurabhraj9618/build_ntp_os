...
dnl
    ...
    if test "$want_mysql" = "yes"; then
        AC_CHECK_HEADERS([mysql.h],[te_header_found="true"], [],
    	[[#include <my_global.h>
     		               ]])
    fi
    ...
    if test "$found_mysql" = "yes" ; then
        ...
        AC_SUBST(SM_REQ_MYSQL, [1])  
	if test "x$MYSQL_CONFIG" == "x"; then
	    RPM_CONFIG_FLAGS="${RPM_CONFIG_FLAGS} --with-mysql=${withval}"
        else
            RPM_CONFIG_FLAGS="${RPM_CONFIG_FLAGS} --with-mysql"
        fi
    fi

    AM_CONDITIONAL(MYSQLENABLE, [test "$found_mysql" = "yes"])
    AC_SUBST([MYSQL_VERSION])
    AC_SUBST([MYSQL_CFLAGS])
    AC_SUBST([MYSQL_LDFLAGS])
])