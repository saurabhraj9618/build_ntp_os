...
AC_DEFUN([PT_SPREAD_NOT_FOUND],[
    echo "WARNING: Spread 4.1 $1 not found - libfixbuf will be built without Spread support"
])
...
AC_DEFUN([AC_PATH_SPREAD],[
    ...
    AC_CHECK_LIB(fixbuf, fBufSetSpreadExportGroup, 
    		[AC_MSG_NOTICE([fixbuf compiled with spread])],
		[AC_MSG_WARN([fixbuf not compiled with spread - not building with spread])
		te_library_found="false"])
    ...
    if test "$te_pthread" == "yes"; then
        if test "$te_library_found" == "true"; then
		    ...
		    RPM_CONFIG_FLAGS="${RPM_CONFIG_FLAGS} --with-spread=${te_lib_path}"
		else
